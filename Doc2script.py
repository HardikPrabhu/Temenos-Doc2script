...
def clr_doc():
    global Document_loc,Documents
    Document_loc=[]
    Documents=0
    label2.config(text=f"Documents: {Documents}")
    status.config(text="Documents removed")
...
def mapping():
    ...
    Button(similar, text="Clear Documents", fg="white", bg="Royalblue2", command=clr_doc).grid(row=7, column=0)
